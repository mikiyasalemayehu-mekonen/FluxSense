  ...
  return (
    <main className="relative min-h-screen overflow-x-hidden bg-[#070A13] text-white">
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        ...
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_top,_rgba(255,255,255,0.08),_transparent_34%),linear-gradient(180deg,rgba(7,10,19,0.3),rgba(7,10,19,0.92))]" />
      </div>
      ...
      <div className="relative mx-auto flex w-full max-w-7xl flex-col gap-10 px-4 py-5 sm:px-6 lg:px-8 lg:py-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-white/45">
                Environmental risk intelligence for decision teams
              </p>
            </div>
          </div>
        ...
        <section className="grid items-center gap-8 lg:grid-cols-[1.06fr_0.94fr] lg:gap-10">
          <div className="space-y-7">
            ...
            <div className="space-y-5">
              <h1 className="font-display max-w-3xl text-5xl font-bold leading-[0.95] tracking-[-0.05em] text-white sm:text-6xl lg:text-7xl">
                Monitor environmental risk with a sharper, faster SaaS workflow.
              </h1>
              <p className="max-w-2xl text-base leading-7 text-white/68 sm:text-lg">
                FluxSense turns satellite imagery, segmentation, detection, and report intelligence into a live command center for environmental monitoring, incident review, and risk planning.
              </p>
            </div>
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              {[
                ...
                <div key={label} className="rounded-2xl border border-white/10 bg-white/6 px-4 py-4 backdrop-blur-sm">
          ...
          <div className="relative">
            ...
            <div className="relative overflow-hidden rounded-[2rem] border border-white/10 bg-slate-950/75 p-5 shadow-2xl shadow-cyan-950/30 backdrop-blur-xl">
                ...
                {[
                  {
                    ...
                    text: "Visual context for the selected region.",
                  },
                  {
                    icon: Activity,
                    title: "Signal fusion",
                    text: "Risk score, detection, and NLP combined.",
                  },
                  {
                    icon: FileText,
                    title: "Evidence trail",
                    text: "Linked sources and report snippets.",
                  },
                  {
                    icon: TrendingUp,
                    title: "Forecast layer",
                    text: "Direction and peak risk timing.",
                  },
              ...
              <div className="mt-5 rounded-2xl border border-white/10 bg-gradient-to-r from-cyan-400/10 via-blue-400/10 to-fuchsia-400/10 p-4">
                <div className="flex items-center justify-between gap-4">
                  <div>
                    <p className="text-xs uppercase tracking-[0.24em] text-white/45">Operational flow</p>
                    <p className="mt-1 text-sm text-white/80">Select a region, run analysis, and review the result in one continuous workspace.</p>
                  </div>
        ...
        <section id="platform" className="grid gap-4 md:grid-cols-3">
          {featureCards.map(({ icon: Icon, title, text }) => (
            <div key={title} className="group rounded-3xl border border-white/10 bg-white/6 p-6 backdrop-blur-xl transition hover:-translate-y-1 hover:border-cyan-400/25 hover:bg-white/8">
        ...
        <section id="workflow" className="grid gap-4 lg:grid-cols-3">
          {steps.map(({ number, title, text }) => (
            <div key={number} className="rounded-3xl border border-white/10 bg-[#0B1020]/80 p-6 backdrop-blur-xl">
              <p className="font-display text-3xl font-semibold tracking-[-0.06em] text-cyan-200">{number}</p>
        ...
        <section id="demo" className="grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
          <div className="overflow-hidden rounded-[2rem] border border-white/10 bg-slate-950/70 shadow-2xl shadow-cyan-950/20 backdrop-blur-xl">
            <div className="flex flex-wrap items-center justify-between gap-4 border-b border-white/10 px-5 py-4">
              <div>
                <p className="text-xs uppercase tracking-[0.28em] text-white/40">Live workspace</p>
                <h2 className="mt-1 text-2xl font-semibold text-white">Draw a region and analyze it instantly</h2>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                >
                  {loading ? (
                    ...
                    <>
                      <Satellite size={16} />
                      {bbox ? "Analyse region" : "Draw a region first"}
                    </>
                  )}
                </button>
              </div>
            ...
            {analysis?.message && (
              <div className="mx-5 mt-5 inline-flex rounded-full border border-white/10 bg-white/6 px-4 py-2 text-xs text-white/70">
                {analysis.message}
              </div>
            )}
            ...
            {analysis && (
              <div className="flex flex-wrap items-center justify-between gap-3 border-t border-white/10 px-5 py-4">
                ...
                <div className="text-xs text-white/45">
                  Toggle the view to compare imagery and segmentation overlays.
                </div>
              </div>
            )}
          </div>

          <aside className="overflow-hidden rounded-[2rem] border border-white/10 bg-[#0B1020]/80 shadow-2xl shadow-slate-950/30 backdrop-blur-xl">
            <div className="border-b border-white/10 px-5 py-4">
              <p className="text-xs uppercase tracking-[0.28em] text-white/40">Insights</p>
              <h2 className="mt-1 text-2xl font-semibold text-white">
                {analysis ? "Analysis results" : "What you get after analysis"}
              </h2>
            </div>
            ...
            {analysis ? (
              <>
                ...
                <div className="max-h-[650px] overflow-y-auto">
                  ...
                  {tab === "forecast" && forecast && <ForecastChart forecast={forecast} />}

                  {tab === "sources" && (
                    <div className="space-y-3 p-4">
                      <p className="text-xs uppercase tracking-[0.24em] text-white/40">Data sources</p>
                      {analysis.nlp.sources.length === 0 ? (
                        <div className="rounded-2xl border border-white/10 bg-white/5 p-4 text-sm text-white/55">
                          No external reports were found for this region.
                        </div>
                      ) : (
                        analysis.nlp.sources.map((source, index) => (
                          <div key={`${source.title}-${index}`} className="rounded-2xl border border-white/10 bg-white/5 p-4">
              ...
              <div className="space-y-5 p-5">
                <div className="rounded-3xl border border-white/10 bg-gradient-to-br from-cyan-400/10 via-blue-400/10 to-fuchsia-400/10 p-5">
                  ...
                  <h3 className="text-xl font-semibold text-white">A more standard SaaS experience</h3>
                  <p className="mt-2 text-sm leading-7 text-white/60">
                    The app now opens with a product-led landing page, while the live map analysis remains available as the core workflow.
                  </p>
                </div>
                ...
                <div className="rounded-2xl border border-white/10 bg-slate-950/60 p-4 text-sm text-white/55">
        ...
        <section className="rounded-[2rem] border border-white/10 bg-white/5 px-6 py-8 backdrop-blur-xl lg:px-8">
          <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
            <div>
              <p className="text-xs uppercase tracking-[0.3em] text-white/40">Designed for operations</p>
              <h2 className="font-display mt-3 text-3xl font-semibold tracking-[-0.04em] text-white sm:text-4xl">
                A polished interface for teams that need environmental intelligence, not a research prototype.
              </h2>
              <p className="mt-4 max-w-2xl text-sm leading-7 text-white/60">
                The experience is structured like a standard SaaS product: strong branding, clear hierarchy, actionable cards, and a live workspace that makes the analysis feel usable in real workflows.
              </p>
            </div>
